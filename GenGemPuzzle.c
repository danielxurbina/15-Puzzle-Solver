...
// 4x4 puzzle with 15 tiles numbered from 1 to 15 and a blank tile 0
struct node {
    int tiles[N][N];
    int f, g, h;
...
// This function prints a node in one line
...
int main(int argc,char **argv) {
    ...
    // check input arguments, if incorrect, print a message to the user
    ...
    srand(time(NULL));
    i=0;
    // generate a random sequence of moves to shuffle the puzzle
    do{
        j = rand();
        k = j % 4;
